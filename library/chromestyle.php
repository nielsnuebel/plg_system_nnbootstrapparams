...
class JFormFieldChromeStyle extends JFormFieldGroupedList
{
	...
	protected function getTemplateModuleStyles()
	{
		...
		$modulesFilePath = __DIR__ . '/styles.php';
		...
		if (file_exists($modulesFilePath))
		{
			...
			if (!array_key_exists('system', $moduleStyles))
			{
				$moduleStyles['system'] = array();
				$stylesToAdd = $styles[1];
			}
			else
			{
				$systemStyles = $moduleStyles['system'];
				$stylesToAdd = array_merge($systemStyles, $styles[1]);
			}


			$moduleStyles['system'] = $stylesToAdd;
		}
